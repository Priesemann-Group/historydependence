...
ax1.plot(d_list[d_opt_index:-1], R_nsb[d_opt_index:],
         '--',  label=r'NSB ', linewidth=1.5, color=main_red)
ax1.plot(d_list[:d_opt_index + 1], R_nsb[:d_opt_index + 1], '-',
         label='BBC', linewidth=1.5, color=main_red, zorder=9)
ax1.fill_between(d_list[:-1], R_plugin - 2 * R_plugin_std,
                 R_plugin + 2 * R_plugin_std, facecolor=main_blue, alpha=0.3)

ax1.fill_between(d_list[:-1], R_nsb - 2 * R_nsb_std, R_nsb + 2 * R_nsb_std, facecolor=main_red, alpha=0.3)

ax1.plot([d_list[d_opt_index]],
         [R_nsb[d_opt_index]], 'd', color=main_red, zorder=10)
ax1.text(6.3, 0.006, r'\begin{center}BBC selects unbiased \\ NSB estimates \end{center}', color='0.35', ha='left', va='bottom')
ax1.legend(loc=(0.05, 0.45), frameon=False)

fig.tight_layout(pad=1.0, w_pad=1.0, h_pad=1.0)
plt.savefig('{}/Fig5A_R_vs_d_bbc.pdf'.format(PLOTTING_DIR),
