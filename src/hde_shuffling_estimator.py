...
def shuffling_MI_with_correction(symbol_counts, number_of_bins_d):
    past_symbol_counts, number_of_symbols, shuffled_past_symbol_counts, marginal_probabilities \
        = get_shuffled_symbol_counts(symbol_counts, number_of_bins_d)
    ...
    return H0_sh_cond - H_cond - H0_cond + H_uncond, H0_cond - H0_sh_cond
...
def shuffling_estimator_with_correction(symbol_counts, number_of_bins_d, H_uncond,
                        return_ais=False):
    I_sh, correction = shuffling_MI_with_correction(symbol_counts,
                        number_of_bins_d)

    if return_ais:
        return I_sh, correction
    else:
        return I_sh / H_uncond, correction / H_uncond
